...
def train(tf_config, logger):
    dataset = data.Dataset(cfg.DATASET, cfg.RNG_SEED, training=True)
    imgs, labels = dataset.preprocessing(
        augment=True, batch_size=cfg.TRAIN.BATCH_SIZE, num_epochs=cfg.TRAIN.EPOCH)

    dataset = data.Dataset('vgg_unlabeled', cfg.RNG_SEED, training=True)
    ...
    with tf.variable_scope('cls', reuse=True):
        density_orig = tf.layers.conv2d(net, 1, 1, activation=tf.nn.relu)
    if 'cutout' in cfg.TRAIN.SEMI_AUG:
    ...
    if cfg.TRAIN.PEAK:
        max_filter = tf.layers.max_pooling2d(
        ...
        if cfg.TRAIN.KERNEL == 'gaussian':
            kernel = utils.gauss2D((5, 5), 1.)[..., np.newaxis, np.newaxis]
            kernel = tf.constant(kernel, tf.float32)
            weights = tf.nn.conv2d(
                weights, kernel, strides=[1, 1, 1, 1], padding='SAME')
    ...
    tf.summary.image('img', imgs[:, :, :, :], max_outputs=1)
    tf.summary.image('img_aug', imgs_aug[:, :, :, :], max_outputs=1)
    tf.summary.image('img_orig', imgs_orig[:, :, :, :], max_outputs=1)
    tf.summary.image('gt', labels[:, :, :, :], max_outputs=1)
    tf.summary.image('pred', density_sup[:, :, :, :], max_outputs=1)
    tf.summary.image('pred_aug', density_aug[:, :, :, :], max_outputs=1)
    tf.summary.image('pred_orig', density_orig[:, :, :, :], max_outputs=1)
...
def test(tf_config, logger):
    ...
    dataset = data.Dataset(cfg.DATASET, cfg.RNG_SEED, training=False)
    imgs, _ = dataset.preprocessing(
        augment=False, batch_size=cfg.TRAIN.BATCH_SIZE, num_epochs=-1)

    net, _ = model.unet(imgs, bn_training=False, layers=4, features_root=32,
